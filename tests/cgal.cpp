...
void testCGALParallel(int Dim, int LEAVE_WRAP, parlay::sequence<point>& wp, int N, int K, const int& rounds,
    ...
    parlay::parallel_for(
        0, N, [&](size_t i) { _points[i] = Point_d(Dim, std::begin(wp[i].pnt), (std::begin(wp[i].pnt) + Dim)); });
    parlay::parallel_for(0, wi.size(), [&](size_t i) {
        _points_insert[i] = Point_d(Dim, std::begin(wi[i].pnt), (std::begin(wi[i].pnt) + Dim));
    });
    ...
    auto run_cgal_knn = [&](const auto& wp, int kth, size_t batchSize) {
        ...
        auto ave_knn = time_loop(
                ...
                tbb::parallel_for(tbb::blocked_range<std::size_t>(0, batchSize),
                                  [&](const tbb::blocked_range<std::size_t>& r) {
                                      for (std::size_t s = r.begin(); s != r.end(); ++s) {
                                          ...
                                          Point_d query(Dim, std::begin(wp[s].pnt), std::end(wp[s].pnt));
            ...
            [&]() {});
        // tbb::parallel_for(tbb::blocked_range<std::size_t>(0, batchSize), [&](const tbb::blocked_range<std::size_t>&
        // r) {
        //     for (std::size_t s = r.begin(); s != r.end(); ++s) {
        //         // Neighbor search can be instantiated from
        //         // several threads at the same time
        ...
        // });
        timer.stop();
        std::cout << ave_knn << " " << tree.root()->depth() << " " << parlay::reduce(visNodeNum) / batchSize << " "
                  << std::flush;
    };
    ...
    if (queryType & (1 << 17)) {  // NOTE: OOD
        ...
        for (int i = 0; i < 3; i++) {
            run_cgal_knn(query_points, k[i], batchSize);
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    using point = PointType<coord, 16>;
    using points = parlay::sequence<point>;
