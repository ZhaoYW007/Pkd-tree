...
void ParallelKDtree<point>::build(slice A, const dim_type DIM) {
...
template<typename point>
void ParallelKDtree<point>::divide_rotate(slice In, splitter_s& pivots, dim_type dim, bucket_type idx, bucket_type deep,
...
template<typename point>
void ParallelKDtree<point>::pick_pivots(slice In, const size_t& n, splitter_s& pivots, const dim_type dim,
...
template<typename point>
inline uint_fast8_t ParallelKDtree<point>::find_bucket(const point& p, const splitter_s& pivots) {
    uint_fast8_t k = 1;
    while (k <= PIVOT_NUM) {
...
template<typename point>
void ParallelKDtree<point>::partition(slice A, slice B, const size_t n, const splitter_s& pivots,
...
template<typename point>
typename ParallelKDtree<point>::node* ParallelKDtree<point>::build_inner_tree(bucket_type idx, splitter_s& pivots,
...
template<typename point>
typename ParallelKDtree<point>::node* ParallelKDtree<point>::serial_build_recursive(slice In, slice Out, dim_type dim,
    ...
    auto parallel_partition = [&](slice In, dim_type d) {
        auto cmp = [&](const point& p1, const point& p2) { return Num::Lt(p1.pnt[d], p2.pnt[d]); };
        ...
        auto res = parlay::internal::split_two(
            In, parlay::delayed_seq<bool>(n, [&](const point& p) { return (*ucmp)(p.pnt[d], iter_pivot->pnt[d]); }));
...
template<typename point>
typename ParallelKDtree<point>::node* ParallelKDtree<point>::build_recursive(slice In, slice Out, dim_type dim,
                                                                             const dim_type DIM, const box& bx) {
    ...
    if (zeros == BUCKET_NUM - 1) {  // NOTE: switch to seral
    ...
    parlay::parallel_for(
        ...
        [&](bucket_type i) {
            ...
            treeNodes[nodesMap[i]] =
                build_recursive(Out.cut(start, start + sums[nodesMap[i]]), In.cut(start, start + sums[nodesMap[i]]),
                                dim, DIM, boxs[nodesMap[i]]);
        },
        1);
